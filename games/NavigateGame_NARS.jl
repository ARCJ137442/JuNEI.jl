...
function inputType()::NARSType
    while true
        try
            return "NARS Type: " |> input |> NARSType
        catch
...
"（对接）请求输入"
function requestInput(game::NavigationGame)::String
    # readline() # 直接读取命令行输入
    ...
    # 遍历所有Agent，搜索其操作
    for (i, agent, operation, num) in iterate_operations(game.env_link)
        # 获取到一个有效的信息
        if num > 0
...
"（对接）游戏反馈（返回：是否「执行移动」）"
function response(game::NavigationGame, move_direction, move_vec)
...
function game_end(game::NavigationGame, move_count)
    println("恭喜，已到达目标点！")
